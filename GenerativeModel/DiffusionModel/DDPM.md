...
4. **单步加噪**
   ...
   3. 可以 继续根据上述步骤 单步加噪
5. 其实也可以 **多步加噪**
   1. 随机变量相互独立(或至少不相关)时，它们的 和或差 的方差 等于各自方差之和，若存在相关性，则需要加上协方差项
6. 目前 加噪过程 存在问题
...
7. DDPM 改进
   ...
   3. 加噪时，对均值进行缩小，多次迭代后，均值才能变为 0
   4. 改进后的 加噪过程 (每个 $\epsilon$，即 $\epsilon_1, \epsilon_2, \dots, \epsilon_T$，都是独立地 从 标准正态分布 中采样的)
   5. <img src="Pics/rethinkfun005.png" width=800>
   6. 从最终结果可以看出，最终 $x_T \sim \mathcal{N}(0,I)$
...
4. 贝叶斯公式
   ...
   2. 无法直接使用，因为 无法计算 $q(x_t)$ & $q(x_{t-1})$ 的 边缘概率密度函数 (**所有可能的原始图片下**，通过 n 步加噪，达到 q 的概率)
   3. 条件贝叶斯定理 : $$ P(A|B, C) = \frac{P(B|A, C) P(A|C)}{P(B|C)}$$
      ...
      3. 应用 **马尔可夫性质** $q(x_t|x_{t-1}, x_0) = q(x_t|x_{t-1})$，$x_{t-1}$ 到 $x_t$ 的加噪过程 只 依赖 $x_{t-1}$，和 $x_0$ 无关
      4. 引入 $x_0$ 后，所有的项 都是 正向过程的 条件概率，都具有高斯分布的解析形式
5. $$q(x_{t-1}|x_t, x_0) = \frac{q(x_t|x_{t-1}) \cdot q(x_{t-1}|x_0)}{q(x_t|x_0)}$$
   1. 当前公式 **需要知道最终答案 $x_0$** 才能生成，用 神经网络 $p_{\theta}(x_{t-1} | x_t)$，拟合 $x_t$ 降噪到 $x_{t-1}$ 的正态分布 $q(x_{t-1}|x_t, x_0)$，也就是 拟合 均值 & 方差
6. $p_{\theta}$ 需要拟合 不同的 原始图片 $x_0$ 从 $x_t$ 降噪到 $x_{t-1}$ 的均值 & 方差，学到的是 通用的、泛化的、不依赖于具体 $x_0$ 的 去噪函数，一种 模式(Pattern)
   1. 在 训练时 利用 $x_0$ 的信息，使用可解析计算的项 $q(x_{t-1}|x_t, x_0)$ 作为模型 GroundTruth
   2. 在 生成时(推理时) 是独立的，不需要 $x_0$，没有答案
...
5. 训练时通过贝叶斯公式计算出 $q(x_{t-1}|x_t, x_0)$ 的均值和方差，然后用 神经网络 $p_{\theta}(x_{t-1}|x_t)$ 去拟合 **均值 & 方差**
6. 训练得到 $p_{\theta}$ 后，就可以通过对一个完全随机的噪声经过逐步去噪生成一个图片

**==DDPM 的去噪 需要 一步步来，不像 加噪 一步到位==**，只有 微小 & 逐步的 去噪，才能保证 模型可以稳定地 用高斯分布来 学习 & 预测
...
2. 对样本 $x^1, x^2, \dots, x^i, \dots, x^n$，得到能让 $\prod p_{\theta}(x^i)$ 尽可能大的 $\theta$
...
3. $$p(x_0, x_1, \dots, x_T) = p(x_T) \cdot p(x_{T-1}|x_T) \cdot p(x_{T-2}|x_{T-1}, x_T) \cdots p(x_0|x_1, \dots, x_T)$$
   1. 另 $x_{0:T} = \{x_0, x_1, \dots, x_T\}$
   2. $x_0$ 表示 原始清晰的图片
4. $$p(x_{0:T}) = p(x_T) \prod_{t=1}^T p(x_{t-1}|x_t, x_{t+1}, \dots, x_T)= p(x_T) \prod_{t=1}^T p_{\theta}(x_{t-1}|x_t)$$

最小化 某个样本的 损失函数 $-\log p_{\theta}(x_0)$
1. 目标
   1. <img src="Pics/rethinkfun008.png" width=900>
2. 对 $1 : T$ 积分，引入 前向加噪过程 的概率，改写为 数学期望 形式，结合 Jensen 不等式
   1. <img src="Pics/rethinkfun009.png" width=400>
3. 带入 正向 & 反向 过程
   1. <img src="Pics/rethinkfun010.png" width=900>
4. 化简，最后 得到 3项，第1项 没有训练参数 不需要优化，第3项 是 去噪 最后一步 已经清晰了
   1. <img src="Pics/rethinkfun011.png" width=900>
5. 主要 优化 中间项
