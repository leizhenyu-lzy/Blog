...
**IP**[240]中断优先级控制寄存器*组*。每个可屏蔽中断占用**8bit**，总共可以表示240个可屏蔽中断，但是STM32F103只用到了IP[59]~IP[0]这60个。IP寄存器的8个bit并没有全部使用，只用了高4位，也就是bit4~bit7位。Bit4~bit7位又分为抢占优先级和响应先级，抢占优先级在前，响应先级在后。而对于抢占优先级和响应优先级各占几位，则需要根据SCB的AIRCR寄存器中的中断分组设置有决定。
