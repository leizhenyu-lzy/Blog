...
![](Pics/timedomain061.png)


![](Pics/timedomain062.png)
...
![](Pics/rootlocus005.png)

闭环零点不会随开环增益改变，但是包括前向通道的开环零点和反馈通道的开环极点。

同时满足模条件和相角条件说明在根轨迹上。（$K^*$是0到∞变化的，所以模条件总是可以满足的，但是相角条件并不一定满足）

![](Pics/rootlocus006.png)

![](Pics/rootlocus007.png)

![](Pics/rootlocus008.png)

![](Pics/rootlocus009.png)
...
![](Pics/rootlocus019.png)

由根之和可知，在本题中，-1和0的分离点应该在-0.5和-1之间（-4和-2的根在向右走），同时也不能超过渐近线。

![](Pics/rootlocus020.png)

分离点至少是一个二重根。

![](Pics/rootlocus021.png)

对于算出的根轨迹，需要进行检验，以保证正确性，不符合的要舍去。

![](Pics/rootlocus022.png)
...
![](Pics/rootlocus023.png)

![](Pics/rootlocus024.png)

在要求的极点附近取一点（挨得很近，开环零点极点到这个点的向量近似等于到所求点的向量）。通过相角条件列写方程，$\Theta_1$就是所要求的出射角。

![](Pics/rootlocus025.png)

![](Pics/rootlocus026.png)

![](Pics/rootlocus027.png)

![](Pics/rootlocus028.png)

![](Pics/rootlocus029.png)

上图可以稳定但是稳定的不多（右边的曲线向左穿过虚轴的时候是稳定的）。但系统的动态特性不好，$\beta$角较大，阻尼比小，超调量大。另外极点实部较小，调节时间较长。

![](Pics/rootlocus030.png)
