...
MuJoCo 的 高性能扩展版本，让 MuJoCo 的 物理仿真 能够 跑在 GPU 和 TPU 上，提升 并行仿真的速度

基于 **==JAX 框架==** 对 MuJoCo 物理算法的重新实现
1. JAX 是由 Google 开发的 用于 高性能数值计算 & 机器学习研究 的 Python 库
2. JAX 特性
   1. NumPy 一样的 API，语法几乎和 NumPy 一模一样
   2. 自动微分 (grad) : 可以对几乎任何 Python/NumPy 函数进行求导
   3. 及时编译 (jit) : 使用 **XLA (Accelerated Linear Algebra) 编译器**，将 Python 代码编译成针对 GPU 或 TPU 优化的机器码，消除 Python 的解释开销，进行 **算子融合**，从而获得接近原生 C++ 的运行速度
   4. 向量化自动映射 (vmap) : 处理单个数据的代码 通过 vmap 转换为 批量处理(Batching) 的代码

MJX 与 MuJoCo 区别
1. 大部分函数名相同，MJX 支持 Batch Dimensions
2. 运算前 需要 将数据从 CPU 搬运到 GPU/TPU 上
3. 碰撞检测
   1. MJX 在处理 **大型复杂网格(Mesh)** 时性能较差，使用的是 SAT(分离轴定理，Separating Axis Theorem) 算法
   2. 建议碰撞网格的顶点数保持在 200 以下
   3. 分支限制，GPU/TPU 不擅长处理复杂的逻辑分支
4. 性能优化建议
   1. 降低 Solver 的迭代次数 (通常 1 次就够强化学习用)
   2. 显式指定哪些物体需要碰撞，避免全局检测
   3. 稀疏/稠密矩阵选择
      1. TPU 推荐使用 sparse(稀疏) 模式
      2. GPU 推荐使用 dense(稠密)  模式
