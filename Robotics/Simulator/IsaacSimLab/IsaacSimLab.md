...
**Terrain Generator** : 根据 sub-terrain 的 config 创建 **trimesh** object，并且包含 origins，还需要 importer 帮助导入 仿真
1. `TerrainGenerator`
   1. `__init__.py`
      1. `np_rng` : random number generator (随机数生成器)，`np.random.default_rng(seed)` (随机数独立)
      2. 最后统一将 各个 SubTerrain & border 的 trimesh **concatenate** 为 完整大的 `terrain_mesh`
   2. `_get_terrain_mesh()`
      1. 一个 SubTerrain 可以由 多个 trimesh 组成，通过 **concatenate** 组合
   3. `_add_sub_terrain()`
      1. 处理 sample flat_patch
         1. `convert_to_warp_mesh`(wp_mesh 为了临时 raycast 用) + `find_flat_patches`(内部会做大量向下 raycast，10000 次)
      2. transform(translate) 到正确位置
   4. `_generate_random_terrains()` : 完全随机
      1. 先 proportions 归一化
      2. `unravel_index` 一维 **线性索引**，还原成 **多维网格坐标**
      3. `np_rng` 随机抽取 terrain_index & difficulty
      4. 调用 `_get_terrain_mesh()` 得到 mesh & origin，调用 `_add_sub_terrain()`
   5. `_generate_curriculum_terrains()` : 每列 分配 同一种 sub_terrain 类型，每行 决定
      1. 先 proportions 归一化
      2. 通过 `np.cumsum` & `np.where` & `np.min` 得到 每一列的 sub_terrain index
      3. 先遍历 cols，再遍历 rows，每个 row 的 difficulty 基本上是 线性增加(有一定的随机扰动)
      4. 调用 `_get_terrain_mesh()` 得到 mesh & origin，调用 `_add_sub_terrain()`
2. `TerrainGeneratorCfg`
   1. `seed` : 用于后续 的 random number generator
   2. `curriculum` : True 即为 curriculum mode(default)，False 为 random 模式，决定 generator 生成 terrain 的方法
   3. `size` : 每一个 sub-terrain 小块的尺寸，后续会 覆盖 sub_terrain 的 size
   4. `border_width` & `border_height` : 最外面的大边框，不同于 sub_terrain(`HfTerrainBaseCfg`) 的 border_width
   5. `num_rows` & `num_cols` : 地图布局规模
   6. `color_scheme`
   7. `horizontal_scale` & `vertical_scale`
   8. `slope_threshold` - TODO
   9. ==☆==`sub_terrains: dict[str, SubTerrainBaseCfg]`
   10. `difficulty_range`
   11. `use_cache` & `cache_dir`
...
2. `TerrainImporterCfg`
   ...
   2. 可以 脱离 `InteractiveScene` 单独使用，就需要 `TerrainImporterCfg` 有自己的 num_envs & env_spacing
   3. 对于 `class InteractiveScene`，在 `_add_entities_from_cfg` 中，有 对于 `TerrainImporterCfg` 的 num_envs & env_spacing 的 override


`sub_terrain_cfg.py`
1. class `SubTerrainBaseCfg` : 所有具体地形配置类的基类
   1. `function: Callable[[float, SubTerrainBaseCfg], tuple[list[trimesh.Trimesh], np.ndarray]]` : 应该调用的 生成函数
      1. **类型标注** 解释 : **接受** difficulty(float) & SubTerrainBaseCfg，**返回** trimesh列表 & origin(np.ndarray)
      2. 一个 terrain 可以由 多个 trimesh 组成，在 `terrain_generator.py` 的 `_get_terrain_mesh()` 中 **concatenate**
   2. `proportion` : 在混合多种 terrain 时，这类 terrain 被采样到的概率权重 (后续还会 统一 在 `TerrainGenerator` 中 归一化)
   3. `size` : 整体尺寸 包含 border (单位 : m)，width(x方向) & length(y方向)，**会被 `TerrainGeneratorCfg` 的 size 覆盖**
   4. `flat_patch_sampling: dict[str, FlatPatchSamplingCfg]` : dict 的原因是 多种采样需求
2. class `FlatPatchSamplingCfg` : 在 sub-terrain 上额外采样一些平坦落脚点(圆形区域)，尝试一定次数，找不到会报错，后续 可作为 机器人出生点
   1. `num_patches` : 采样的 平坦 patch 数量
   2. `patch_radius` : patch 的半径，找一个局部区域，需要足够平
   3. `x/y/z_range` : 限制 patch 采样范围 (单位 : m)
   4. `max_height_diff` : 一个 patch 内允许的最大高度差，平坦 判据



**height_field** folder : 先用 高度图 描述地形，再转成 mesh
1. `hf_terrains_cfg.py`
   1. class `HfTerrainBaseCfg`(SubTerrainBaseCfg) : border_width, horizontal/vertical_scale, slope_threshold - TODO
      1. class `HfRandomUniformTerrainCfg`
      2. class `HfPyramidSlopedTerrainCfg`
         1. class `HfInvertedPyramidSlopedTerrainCfg`
      3. class `HfPyramidStairsTerrainCfg`
         1. class `HfInvertedPyramidStairsTerrainCfg`
      4. class `HfDiscreteObstaclesTerrainCfg`
      5. class `HfWaveTerrainCfg`
      6. class `HfSteppingStonesTerrainCfg`
2. `hf_terrains.py`
   1.
3. `utils.py`
   1. `height_field_to_mesh` : **Decorator 装饰器**，被 wrap 的 function 需要 `difficulty` & `cfg`
      1. cfg 的 size 包含 width & length
   2. `convert_height_field_to_mesh` : 实际实现 height-field array 转为 trimesh 功能
      1.

**trimesh** folder : 直接生成 mesh primitive
1. `mesh_terrains_cfg.py`
   1. class `MeshPlaneTerrainCfg`(SubTerrainBaseCfg)
   2. class `MeshPyramidStairsTerrainCfg`(SubTerrainBaseCfg)
      1. MeshInvertedPyramidStairsTerrainCfg`
   3. class `MeshRandomGridTerrainCfg`
   4. class `MeshRailsTerrainCfg`
   5. class `MeshPitTerrainCfg`
   6. class `MeshBoxTerrainCfg`
   7. class `MeshGapTerrainCfg`
   8. class `MeshFloatingRingTerrainCfg`
   9. class `MeshStarTerrainCfg`
   10. class `MeshRepeatedObjectsTerrainCfg`
       1. class `MeshRepeatedPyramidsTerrainCfg`
       2. class `MeshRepeatedBoxesTerrainCfg`
       3. class `MeshRepeatedCylindersTerrainCfg`
2. `mesh_terrains.py`
   1.
3. `utils.py`
   1. `make_plane` : 生成一个矩形平面 mesh
   2. `make_border` : 生成 中间挖空的矩形边框
   3. `make_box` : 生成一个 box 障碍物，并且可以随机倾斜
   4. `make_cylinder` : 生成一个圆柱障碍物，支持随机倾斜
   5. `make_cone` : 生成圆锥 / 金字塔类障碍物，支持随机倾斜
