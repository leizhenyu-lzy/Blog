...
def getPose2DWorld3D(poseDetector, frame_rgb):
    mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=frame_rgb)
    results = poseDetector.detect(mp_image)

    if len(results.pose_world_landmarks) > 0:
        landmarks_world = results.pose_world_landmarks[0]  # (33,)
        points3d = np.array([[lm.x, lm.y, lm.z] for lm in landmarks_world])  # (33, 3)

        landmarks_image = results.pose_landmarks[0]
        points2d = np.array([[lm.x, lm.y] for lm in landmarks_image])  # (33, 2)
...
if __name__ == "__main__":
    modelPath = os.path.join(os.path.dirname(__file__), r"../Models/pose_landmarker_heavy.task")
    ...
    videoPath = os.path.join(os.path.dirname(__file__), r"../Pics/hao.mp4")
    ...
    while True:
        ...
        points3d, points2d = getPose2DWorld3D(poseDetector, frame_rgb)
        ...
        if points3d is not None:
            ...
            # udpate 3d image
            update3Dplot(ax, points3d, MEDIAPIPE_POSE_CONNECTIONS)
            fig.canvas.draw()
