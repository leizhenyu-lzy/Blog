...
数据湖(Data Lake)
1. 一种**存储结构化、半结构化(word/excel/xml)和非结构化(图片/视频)数据的大型存储库**
2. 通常**以原始形式保存数据**(未加工)，而不需要预定义的模式或模型
3. 数据湖本身需要提供一个统一对外的存储访问的接口，解决数据怎样存入和读出的问题，本质是一个存储模式
4. 具有高度的扩展性和灵活性，可以容纳各种类型和格式的数据，包括文本、图像、日志文件等(多模态)
5. 由于数据湖本身支持各种各样的数据存储，同时需要具备弹性扩展能力，所以一般会使用分布式的文件存储或是分布式的对象存储来构建数据湖
6. 通常与大数据技术和分布式处理框架(Hadoop、Spark)结合使用，支持大规模数据处理、数据探索和机器学习等应用
