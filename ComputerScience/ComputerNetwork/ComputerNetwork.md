...
用户自定义 HTTP 回调函数(user-defined HTTP callback)

用于监听某些行为事件，当事件触发时会向用户指定的目标地址发送信息
...
Webhook URL 是公开的，任何知道 URL 的人 都可以 发送伪造请求

保护手段
1. 自定义关键词 : 只推送包含特定关键词的消息
2. IP 白名单 : 只推送由白名单 IP 发送的消息
3. 密钥签名 : 只推送带有指定密钥信息的消息


Polling(轮询) & Webhook
1. Polling
   1. 程序 主动、定时 问服务器 (`while` 循环，不停地主动发起 HTTP 请求)
   2. Pros
      1. 实现简单，客户端主动控制节奏
      2. 无需公网 IP
      3. 断联 可靠性
   3. Cons
      1. 浪费请求
      2. 实时性 取决于 轮询间隔 (间隔短 增加系统压力，间隔长 会变慢)
2. Webhook
   1. 服务端主动推送事件，先提供一个 HTTP 地址(回调 URL)
   2. 程序确实在运行，但处于 **阻塞挂起 / 事件驱动** 状态
   3. 被动内核通知(等待网卡中断信号)
   4. Pros
      1. 更实时，事件发生后通常立即推送
      2. 省资源，不需要持续空查，在事件发生前，双方几乎没有任何网络流量
      3. 适合 **事件驱动场景**，比如消息通知、审批回调、支付结果通知
   5. Cons
      1. 需要可访问的回调接口
      2. 需要处理签名校验、重试、幂等、安全问题
      3. 服务临时不可用，可能会丢消息，依赖平台重试机制
