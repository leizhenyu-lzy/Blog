...
class Agent():
    ...
    def __init__(self,OOXX_Index,Epsilon,Alpha):
        ...
        self.value = np.zeros((3,3,3,3,3,3,3,3,3)) # 储存状态价值的表  # 状态向量可以直接作为价值矩阵的索引
    ...
    def move(self,State):
        ...
        if np.random.binomial(1,self.epsilon): # 判断要不要采取 ε-Greedy 的随机行动
            ...
            for i in range(len(available)): # 遍历每一个可能落子的地方
...
for i in range(Trial):
    ...
    # 默认 Agent1 先行
    # 并且以 Agent1 的 视角定义 State 和 Outcome
    while winner == 0: #如果胜负未分
        ...
        if winner == 1: # 如果 Agent1 获胜
            ...
            # 上一次 agent2 下完 之后 的结果是 state，让 这次 agent1 获得胜利了，因此任务 上次的 结果 不好
