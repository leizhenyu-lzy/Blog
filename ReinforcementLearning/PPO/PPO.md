...
<img src="Pics/rethinkfun000.png" width=300>
...
需要最大化 Return的 期望，使用梯度上升的方法，**对 $\theta$(网络参数) 求导**，最终优化的是动作的概率分布
...
<img src="Pics/rethinkfun002.png" width=400>

将 Trajectory 拆分为 单步概率

最后一行可以理解为 Loss 的 相反数，Loss 需要下降，所有这里需要梯度上升

$\theta \rightarrow \theta + \alpha R(\tau) \nabla \log P_\theta(a_t | s_t)$

trajectory 得到的 return 大于0
...
**补充 : 具体实现**
1. 用 **当前冻结的策略** 在环境里并行跑 若干条 轨迹，该阶段策略 不更新，保持稳定采样
   1. 每条轨迹遇到提早终止事件(机器人摔倒 / 时间上限 T 步)就结束
...
3. 批量更新
   1. 上述 batch 按小批 (mini-batch) 随机打乱，重复 K 个 epoch
      1. 用剪切损失 L_clip 更新策略网络
      2. 用均方误差更新价值网络
   2. 更新完毕即得到 新策略 $\pi'$
4. 再用 $\pi'$ 重新 rollout 收集新数据，重复上述步骤
...
<img src="Pics/rethinkfun008.png" width=700>
...
公式合集
1. <img src="Pics/rethinkfun010.png" width=350>
...
可以使用 **重要性采样**，**将 On-Poliy 替换为 Off-Policy**，可以理解为 用历史数据 更新 当前 policy 时，强度要调整
...
<img src="Pics/rethinkfun013.png" width=650>
